<div class="whatsapp-container">
    <!-- SIDEBAR -->
    <div class="sidebar">
        <div class="sidebar-header">
            <button class="menu-toggle" id="menuToggle" aria-label="Menu">☰</button>
            <h5>Conversas</h5>
            <div style="display: flex; align-items: center; gap: 8px;">
                <!-- <label for="statusSelect">Você está:</label> -->
                <select id="statusSelect" class="status-select" title="Seu status atual">
                    <option value="online">Online</option>
                    <option value="offline">Offline</option>
                    <option value="pausa">Pausa</option>
                </select>

                <div class="text-center p-2">
                    <a href="../api/logout.php" class="btn-logout" title="Sair">🚪</a>
                </div>
            </div>
        </div>
        <div class="conversations-list" id="listaAtendimentos">
            <div class="text-muted text-center p-4">Carregando...</div>
        </div>
    </div>

    <!-- ÁREA DO CHAT -->
    <div class="chat-area" id="chatArea" style="display: none;">
        <div class="chat-header" id="chatHeader"></div>
        <div class="messages-container" id="messagesContainer"></div>
        <div class="chat-input">
            <input type="text" id="msgInput" class="form-control" placeholder="Digite sua mensagem...">
            <button onclick="enviarMsg()">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16">
                    <path d="M15.854.146a.5.5 0 0 1 .11.54l-5.819 14.547a.75.75 0 0 1-1.329.124l-3.178-4.995L.643 7.184a.75.75 0 0 1 .124-1.33L15.314.037a.5.5 0 0 1 .54.11z" />
                </svg>
            </button>
        </div>
    </div>
    <div id="emptyChat" class="empty-chat">
        <div>Selecione uma conversa para começar</div>
    </div>
</div>

<div class="sidebar-overlay" id="sidebarOverlay"></div>
<div class="toast-container" id="toast-container"></div>